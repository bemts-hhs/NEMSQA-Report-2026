# IOWA NEMSQA REPORT TRAUMA-14 2026 ------------------------------------
...
#### population results for 2021-2025 ----
trauma_14_missings <- trauma_14_pop$missingness

### get trauma_14 population data for each year using purrr ------

# track progress
tictoc::tic(msg = "trauma_14_pop_years_init")

trauma_14_pop_years_init <- purrr::map(
  report_years,
  \(yr) {
...
trauma_14_pop_years <- add_year_to_nested(
  x = trauma_14_pop_years_init,
  file = "filter_process",
  years = 2021:2025
)

#### append years to the missingness files ----
trauma_14_missingness_years <- add_year_to_nested(
  x = trauma_14_pop_years_init,
  file = "missingness",
  years = 2021:2025
)
...
trauma_14_pop_years |>
  ...
  population_statistical_file_gt(measure = "Trauma-14") |>
...
#### counties years ----
trauma_14_result_counties_years <- nemsqar::trauma_14(
  df = NULL,
  patient_scene_table = patient_scene_table,
  response_table = response_table,
  situation_table = situation_table,
...
export_nemsqa_data(
  pattern = "trauma_14_(missings|missingness)",
  measure = "Trauma-14",
  folder = "missings"
)
